  ...
  try {
    ...
    if (
      ...
      const mailOptions = {
        ...
        subject: `Votre projet est accepté !`,
        html: `<h2>Bonjour ${devis.clientInfo.name},</h2>
          <p>Votre devis <strong>#${devis.devisNumber}</strong> a été accepté.</p>
          <p>Vous serez recontacté pour la suite du projet.</p>
    ...
    return NextResponse.json({ error: 'Erreur lors de l\'acceptation', details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}
